...
parser.add_argument('--data_dir', default="ls ../nas_benchmarks-development/"
...
class MyWorker(Worker):
    def compute(self, config, budget, **kwargs):
        ...
        print("PRINTING WORKER LOSS: ", str(y))
...
for run_id in range(runs):
    ...
    fh = open(os.path.join(output_path, 'run_%d.pkl' % run_id), 'wb')
    pickle.dump(util.extract_results_to_pickle(results), fh)
    pickle.dump(results, fh)
    fh.close()

    # res = {}
    # res['regret_validation'] = np.arange(start = 1, stop=traj + 1, step=1).tolist()
